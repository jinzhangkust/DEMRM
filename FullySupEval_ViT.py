...
def parse_option():
    ...
    # model dataset
    parser.add_argument('--model_name', type=str, default='FullySupLinearViT')
    # checkpoint
...
class TailingSensorSet(Dataset):
    def __init__(self, train_mode, imsize=224):
        ...
        if transform is None:
            ...
            if train_mode == "train":
                self.transforms = torchvision.transforms.Compose([
                    ...
                    transforms.RandomApply([GaussianBlur([0.1, 1.5])], p=0.7),
                    transforms.RandomHorizontalFlip(),
                    transforms.ToTensor(),
                    normalize])
            else:
                self.transforms = torchvision.transforms.Compose([
                    torchvision.transforms.CenterCrop(self.imsize),
                    transforms.RandomAdjustSharpness(sharpness_factor=2),
                    transforms.RandomApply([GaussianBlur([0.1, 1.5])], p=0.7),
                    transforms.RandomHorizontalFlip(),
                    transforms.ToTensor(),
...
class LinearSensor(nn.Module):
    def __init__(self, opt):
        ...
        self.feature = torchvision.models.vit_b_16(weights=None)
...
def set_loader(opt):
    full_data = TailingSensorSet(train_mode="train", imsize=224)
...
def set_model(opt):
    ...
    if opt.epoch > 50:
        checkpoints = os.path.join(opt.save_folder, 'FullySupLinearViT_epoch_{epoch}.pth'.format(epoch=opt.epoch))
...
def main():
    best_acc = 0
    opt = parse_option()
    tb = SummaryWriter(comment="FullySupLinearViT")
    ...
    for epoch in range(opt.epoch, opt.total_epochs + 1):
        ...
        if epoch % opt.save_freq == 0 and epoch >= 100:
            save_file = os.path.join(opt.save_folder, 'FullySupLinearViT_epoch_{epoch}.pth'.format(epoch=epoch))
