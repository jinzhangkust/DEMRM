...
def parse_option():
    ...
    # model dataset
    parser.add_argument('--model_name', type=str, default='FullySupLinearDEFIE')
    # checkpoint
...
class LinearSensor(nn.Module):
    ...
    def forward(self, x0, x1):
        x1 = self.feature(x1)
        x = torch.cat((x0.view(x0.size(0), -1), x1), dim=1)
        x = self.predictor(x)
        return x
...
def set_model(opt):
    ...
    if opt.epoch > 50:
        checkpoints = os.path.join(opt.save_folder, 'FullySupLinearDEFIE_epoch_{epoch}.pth'.format(epoch=opt.epoch))
...
def main():
    best_acc = 0
    opt = parse_option()
    tb = SummaryWriter(comment="FullySupLinearDEFIE")
    ...
    for epoch in range(opt.epoch, opt.total_epochs + 1):
        ...
        if epoch % opt.save_freq == 0:
            save_file = os.path.join(opt.save_folder, 'FullySupLinearDEFIE_epoch_{epoch}.pth'.format(epoch=epoch))
